...
object Unique {
    ...
    fun getRoundDataFromLocal(activity: Context, valueKey: String?): String? {
        val roundDataHolder =
            activity.getSharedPreferences(CachedHolder.CachedKey.round_mainKey, Context.MODE_PRIVATE)
        return roundDataHolder.getString(valueKey, "")
    }

    fun setRoundDataHolder(activity: Context,valueKey: String?,value: String?){
        val roundDataHolder = activity.getSharedPreferences(CachedHolder.CachedKey.round_mainKey, Context.MODE_PRIVATE)
    ...
    fun View.roundDismissKeyboardFrom(activity: Activity) {
        val round_m = activity.getSystemService(Activity.INPUT_METHOD_SERVICE) as InputMethodManager
